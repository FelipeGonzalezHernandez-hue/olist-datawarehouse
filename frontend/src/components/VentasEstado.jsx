...
export default function VentasEstado({ apiUrl }) {
  const [datos, setDatos] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    fetch(`${apiUrl}/api/ventas-estado`)
      .then(r => r.json())
  ...
  return (
    <div style={card}>
      <h3 style={titulo}>📍 Ventas por Estado</h3>
      <p style={subtitulo}>Volumen de ventas (BRL) agrupado por estado de Brasil</p>
